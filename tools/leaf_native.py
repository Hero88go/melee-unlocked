...
def main():
    ...
    text = ['// GENERATED by tools/leaf_native.py from the recompiler\'s translation. Do not edit, do not commit.',
            ...
            'extern "C" {'] + [decl + ";" for decl in BRIDGES.values()] + [
            "// The estimates themselves, for the game's own square root (math_native.h).",
            "double mu_frsqrte(double v) { return ppc::frsqrte(v); }", "double mu_fres(double v) { return ppc::fres(v); }",
            "}", "", "namespace {"] + pools + [
