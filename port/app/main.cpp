...
// --rng-seed for the recompiled guest: force RNG at the first GS_VS frame callback
// (0x8016D800), after scene setup and immediately before match simulation. ppc::set_hook
// (dispatch-table only) cannot reach a statically-known function call; ppc::add_entry_hook rides
// every translated function's enter() call and sees direct calls too.
static void install_rng_seed_hook() {
  ...
  ppc::add_entry_hook(0x8016D800u, [](ppc::Context&) {
    ...
    host::log("rng-seed: forced %08X at retrace %u (first GS_VS frame)",
