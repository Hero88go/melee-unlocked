...
#pragma once
#include <string>
#include "gx_d3d12.h"   // D3D12Options is the shared render option block

namespace gx {

// Returns nullptr (after logging why) when D3D11 cannot be started, so the caller can fall back.
Backend* create_d3d11_backend(void* hwnd, int client_w, int client_h, const D3D12Options& options);
const D3D12Options& d3d11_options(Backend* backend);
