...
Decision Lifecycle::matchmaking_result(bool connection_success, bool local_ready, bool remote_ready,
  ...
  // Keep the player in offline gameplay during preflight. The native poll continues to collect
  // both players' character-ready state; only leave practice once the peer is ready too.
  if (!connection_success) return {};
  if (!local_ready || !remote_ready) return {};
