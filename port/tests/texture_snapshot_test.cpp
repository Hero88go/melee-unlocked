...
int main() {
  ...
  std::vector<uint8_t> versioned(4096, 3);
  auto stable = cache.capture(versioned.data(), versioned.size(), nullptr, 0, 10, 0);
  check(cache.capture(versioned.data(), versioned.size(), nullptr, 0, 10, 0) == stable,
        "unchanged RAM generation did not reuse the snapshot");
  versioned[2048] = 4;
  check(cache.capture(versioned.data(), versioned.size(), nullptr, 0, 11, 0) != stable,
        "changed RAM generation did not revalidate source bytes");
