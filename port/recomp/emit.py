...
class Emitter:
    ...
    def emit_function(self, info):
        ...
        # instruction, so retail code never does this, but hand-written code patched into the
        # image ignores the symbol map's boundaries: an m-ex build splits an `addis`/`ori` pair
        # across the end of one function and the start of the next. Without this the translated
        # function simply returns there, and the guest takes a return it never asked for.
